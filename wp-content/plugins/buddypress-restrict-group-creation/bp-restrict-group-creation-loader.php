...
Version: 1.1.2
Text Domain: buddypress-restrict-grouo-creation
Network: true
*/
...
function etivite_bp_restrictgroups_add_admin() {
	...
	add_submenu_page( 'options-general.php', __( 'Restrict Group Creation Admin', 'buddypress-restrict-grouo-creation' ), __( 'Restrict Group Creation', 'buddypress-restrict-grouo-creation' ), 'manage_options', 'buddypress-restrict-grouo-creation-settings', 'etivite_bp_restrictgroups_admin' );	
}
...
function etivite_bp_restrictgroups_admin_add_action_link( $links, $file ) {
	...
	if ( function_exists( 'bp_core_do_network_admin' ) ) {
		$settings_url = add_query_arg( 'page', 'buddypress-restrict-grouo-creation-settings', bp_core_do_network_admin() ? network_admin_url( 'admin.php' ) : admin_url( 'admin.php' ) );
	} else {
		$settings_url = add_query_arg( 'page', 'buddypress-restrict-grouo-creation-settings', is_multisite() ? network_admin_url( 'admin.php' ) : admin_url( 'admin.php' ) );
	}

	$settings_link = '<a href="' . $settings_url . '">' . __( 'Settings', 'buddypress-restrict-grouo-creation' ) . '</a>';

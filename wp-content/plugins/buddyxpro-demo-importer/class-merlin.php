...
class Merlin {
	...
	function __construct( $config = array(), $strings = array() ) {
		...
		$config = wp_parse_args(
			$config, array(
				'base_path'            => BDI_PLUGIN_PATH,
				'base_url'             => BDI_PLUGIN_URL,
				'directory'            => 'merlin',
				'merlin_url'           => 'merlin',
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['csframework'] ) ) {
			$content['csframework'] = array(
				...
				'description'      => esc_html__( 'Sample csframework data.', 'merlin-wp' ),
				'pending'          => esc_html__( 'Pending', 'merlin-wp' ),
				'installing'       => esc_html__( 'Installing', 'merlin-wp' ),
				'success'          => esc_html__( 'Success', 'merlin-wp' ),
				'install_callback' => array( 'Merlin_CSFramework_Importer', 'import' ),
				'checked'          => $this->is_possible_upgrade() ? 0 : 1,
	...
	public function get_import_files_paths( $selected_import_index ) {
		...
		if ( ! empty( $selected_import_data['import_cs_framework_file_url'] ) ) {
			...
			$csframework_filename = 'csframework-' . $base_file_name . '.json';

			// Retrieve the content import file.
			$import_files['csframework'] = $downloader->fetch_existing_file( $csframework_filename );

			// Download the file, if it's missing.
			if ( empty( $import_files['csframework'] ) ) {
				$import_files['csframework'] = $downloader->download_file( $selected_import_data['import_cs_framework_file_url'], $csframework_filename );
			}

			// Reset the variable, if there was an error.
			if ( is_wp_error( $import_files['csframework'] ) ) {
				$import_files['csframework'] = '';
			}
		} elseif ( ! empty( $selected_import_data['local_import_cs_framework_file'] ) ) {
			if ( file_exists( $selected_import_data['local_import_cs_framework_file'] ) ) {
				$import_files['csframework'] = $selected_import_data['local_import_cs_framework_file'];
			}
		}

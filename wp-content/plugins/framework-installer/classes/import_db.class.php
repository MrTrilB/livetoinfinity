...
class Toolset_Framework_Installer_Import_Db extends Toolset_Framework_Installer_Install_Step {
	...
	function fix_layouts_images( ) {
		...
		$search = str_replace( '/', '\/', $this->site_url . '/files/' );
	...
	function update_woocommerce_views(){
		...
		foreach( $all_plugins as $plugin_name => $plugin_info ) {
			if ( $plugin_info['Name'] == 'Toolset WooCommerce Blocks' ) {
	...
	function database_recursive_array_replace( $replace_path_placeholder = '' ) {
		...
		//replace site URLS without http or https before.
		$search_for[] = preg_replace('(https:|http:)', '', $this->current_site->site_url );
		$replace_with[] = preg_replace('(https:|http:)', '', $this->site_url );

		$search_for[] = str_replace('/','\/', $this->current_site->site_url );
		$replace_with[] = str_replace('/','\/', $this->site_url );

		$search_for[] = str_replace('/','\/', '/files/' );
		$replace_with[] = str_replace('/','\/', $upload_dir );

...
abstract class Toolset_Framework_Installer_Install_Step {
	...
	/**
	 * @var Toolset_Framework_Installer Framework Installer instance.
	 */
	protected $framework_installer;
	...
	public function __construct(){
		...
		$this->theme = '';
		$this->theme_parent = '';
		$this->framework_installer = $frameworkinstaller;
	...
	/**
	 * Log installation on toolset.com
	 */
	...
	/**
	 * Generate the response for an individual step.
	 *
	 * @param bool $success
	 * @param string $message
	 * @param string $nonce
	 * @return array
	 * @deprecated Use generate_respose instead.
	 */
	function generate_respose_error( $success, $message, $nonce = '' ) {
		return $this->generate_respose( $success, $message, $nonce );
	}
	 ...
	 * @param bool $success
	 * @param string $message
	 * @param string $nonce Deprecated and not used anywhere.
	 * @return array
	 */
	function generate_respose( $success, $message, $nonce = '' ) {
		$data = array(
	...
	function connection_test() {
		...
		$status = $this->framework_installer->download_file( $test_file, $this->upload_dir['basedir'] . '/speed_test.txt' );
		if ( ! $status ) {
			return false;
		}

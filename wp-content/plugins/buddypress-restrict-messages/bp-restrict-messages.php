...
function etivite_bp_restrict_messages_setup_nav() {
	...
	if ( isset( $data['mode']['user']['enabled'] ) )
		return;
...
function venutius_admin_bar_add() {
	...
	$messages_settings_page = bp_core_get_user_domain( $user_id ) . BP_SETTINGS_SLUG . '/messages';
	
	$wp_admin_bar->add_menu( array(
		'parent' => 'my-account-' . BP_SETTINGS_SLUG,
		'id'     => 'restrict-messages',
		'title'  => __( 'Messages', 'bp-restrict-messages' ),
...
function etivite_bp_restrict_messages_settings_menu() {
	...
	if ( isset( $_POST['submit'] ) && wp_verify_nonce( $_POST['etivite_bp_restrict_messages_user_data'], 'etivite_bp_restrict_messages_user_submit' ) ) {
		...
		if( isset( $_POST['rg_pref_groups_status'] ) && !empty( $_POST['rg_pref_groups_status'] ) ) {
			...
			$groups_status = sanitize_text_field( $_POST['rg_pref_groups_status'] );
...
?>
		<form action="<?php echo bp_displayed_user_domain() . $bp->settings->slug . '/messages/' ?>" name="bp-restrict-messages-settings-form" id="bp-restrict-messages-settings-form" method="post">			
				...
				<p class="description"><?php echo esc_attr( __( 'Select who can send you private messages.', 'bp-restrict-messages' ) ); ?></p>
				<table class="form-table">
					<tr>
						<th><label for="rg_pref_friends"><?php esc_attr( _e('Friends Only', 'bp-restrict-messages') ) ?></label></th>
					...
					<?php if ( bp_is_active( 'follow' ) ) { ?>
						<tr>
							<th><label for="rg_pref_friends"><?php esc_attr( _e('Follow Connection Only', 'bp-restrict-messages') ) ?></label></th>
							<td><input <?php if ( $data && $data['follow']['enabled'] ) { echo 'checked'; } ?> type="checkbox" name="rg_pref_follow" id="rg_pref_follow" value="1" /></td>
						</tr>
					<?php } ?>
					<tr>
						<th><label for="rg_pref_groups"><?php esc_attr( _e('Groups Connections Only', 'bp-restrict-messages') ) ?></label></th>
						<td><input <?php if ( $data && $data['samegroup']['enabled'] ) { echo 'checked'; } ?> type="checkbox" name="rg_pref_groups" id="rg_pref_groups" value="1" /></td>
					</tr>
					<tr>
						<td colspan="2">
							<h4><?php echo sanitize_text_field( __( 'Group Status', 'bp-restrict-messages' ) ) ?></h4>
							<p class="description"><?php echo esc_attr( __( 'Select the group status level', 'bp-restrict-messages' ) ) ?></p>
							<table class="form-table">
								<tr>
									<th><label for="rg_pref_groups_status"><?php esc_attr( _e('Public','bp-restrict-messages') ) ?></label></th>
								...
								<tr>
									<th><label for="rg_pref_groups_status_1"><?php esc_attr( _e('Private','bp-restrict-messages') ) ?></label></th>
								...
								<tr>
									<th><label for="rg_pref_groups_status_2"><?php esc_attr( _e('Hidden','bp-restrict-messages') ) ?></label></th>

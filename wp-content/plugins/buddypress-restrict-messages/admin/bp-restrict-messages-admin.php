...
?>	
	<div class="wrap">
		<h2><?php echo sanitize_text_field( __( 'Restrict Messages Admin', 'bp-restrict-messages' ) ); ?></h2>
		...
		<form action="<?php echo $url_base ?>" name="bp-restrict-messages-settings-form" id="bp-restrict-messages-settings-form" method="post">			
			...
			<h3><?php sanitize_text_field( __( 'Mode', 'bp-restrict-messages' ) ) ?></h3>
			<p class="description"><?php echo sanitize_text_field( __( 'Select if site admin or users can set private messaging restrictions', 'bp-restrict-messages' ) ) ?></p>
			<table class="form-table">
				<tr>
					<th><label for="rg_mode"><?php echo sanitize_text_field( __('Admin','bp-restrict-messages' ) ) ?></label></th>
					<td><input type="radio" <?php if ( $data['mode']['sitewide']['enabled'] ) { echo 'checked="checked"'; } ?> name="rg_mode" id="rg_mode" value="admin" /></td>
				</tr>
				<tr>
					<th><label for="rg_mode_1"><?php echo sanitize_text_field( __( 'User','bp-restrict-messages' ) ) ?></label></th>
			...
			<div id="admin-pref" <?php if ( !$data['mode']['sitewide']['enabled'] ) { echo 'style="display:none"'; } ?>>
				<h3><?php echo sanitize_text_field( __( 'Restrictions', 'bp-restrict-messages' ) ) ?></h3>
				<p class="description"><?php echo sanitize_text_field( __( 'Select the private message restrictions.', 'bp-restrict-messages' ) ) ?></p>
				<table class="form-table">
<?php
/*
					<tr>
...
?>
					<tr>
						<th><label for="rg_pref_friends"><?php echo sanitize_text_field( __('Friends Only', 'bp-restrict-messages') ) ?></label></th>
					...
					<?php if ( bp_is_active( 'follow' ) ) { ?>
						<tr>
							<th><label for="rg_pref_friends"><?php echo sanitize_text_field( __('Follow Connection Only', 'bp-restrict-messages') ) ?></label></th>
					...
					<tr>
						<th><label for="rg_pref_groups"><?php echo sanitize_text_field( __( 'Groups Only', 'bp-restrict-messages' ) ) ?></label></th>
						<td><input <?php if ( $data['pref']['samegroup']['enabled'] ) { echo 'checked'; } ?> type="checkbox" name="rg_pref_groups" id="rg_pref_groups" value="1" /></td>
					</tr>
					<tr>
						<td colspan="2">
							<h4><? echo sanitize_text_field( __( 'Group Status', 'bp-restrict-messages' ) ) ?></h4>
							<p class="description"><?php echo sanitize_text_field( __( 'Select the group status level', 'bp-restrict-messages' ) ) ?></p>
							<table class="form-table">
								<tr>
									<th><label for="rg_pref_groups_status"><?php echo sanitize_text_field( __( 'Public','bp-restrict-messages' ) ) ?></label></th>
								...
								<tr>
									<th><label for="rg_pref_groups_status_1"><?php echo sanitize_text_field( __( 'Private','bp-restrict-messages' ) ) ?></label></th>
								...
								<tr>
									<th><label for="rg_pref_groups_status_2"><?php echo sanitize_text_field( __( 'Hidden', 'bp-restrict-messages' ) ) ?></label></th>

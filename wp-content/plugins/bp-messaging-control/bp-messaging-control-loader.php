...
function bpmc_messaging_control_maybe_load_scripts() {
	...
	if ( $current_user_control->get_mentions_character_limit() != 100000 &&// Load the scripts on Activity pages
		...
		(defined('BP_ACTIVITY_SLUG') && 'page' == get_site_option('show_on_front') && is_front_page() && BP_ACTIVITY_SLUG == get_site_option('page_on_front'))
		||
...
function bpmc_messaging_control_enqueue_activity_scripts() {
	...
	wp_localize_script('bp-messaging-control-js', 'BPmcMessCntrl', array(
		...
		'type'      			=> 'char',
		'characterLimitText'	=> sanitize_text_field( __( 'Character Limit: ', 'bp-messaging-control' ) )
	));
}
...
function bpmc_bp_messaging_control_admin_add_admin_menu() {
	...
	add_submenu_page( 'options-general.php', __( 'Messaging Control Admin', 'bp-messaging-control' ), __( 'Messaging Control', 'bp-messaging-control' ), 'manage_options', 'bp-messaging-control-settings', 'bpmc_bp_messaging_control_admin' );
...
function bpmc_bp_messaging_control_admin_add_action_link( $links, $file ) {
	...
	$settings_link = '<a href="' . $settings_url . '">' . __( 'Settings', 'bp-messaging-control' ) . '</a>';

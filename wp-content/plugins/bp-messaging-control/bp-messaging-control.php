...
function bpmc_inform_user() {
	...
	$character_message = sanitize_text_field( __( 'You have unrestricted message length', 'bp-messaging-control' ) );
	
	if ( $current_user_control->get_character_limit() != 100000 ) {
		$character_message = sprintf( sanitize_text_field( __( 'Your messages are limited to %d characters.', 'bp-messaging-control' ) ), $current_user_control->get_character_limit() );
	}
	
	if ( $current_user_control->is_admin_only() ) {
		echo sanitize_text_field( __( 'You can only message the site administrator, ', 'bp-messaging-control' ) ) . $character_message;
	} else if ( $current_user_control->is_reply_only() && ! $current_user_control->is_capped() ) {
		echo sanitize_text_field( __( 'Your messaging is set to Reply Only, you can only message users who have previously sent you a message', 'bp-messaging-control' ) ) . $character_message;
	} else if ( $current_user_control->is_reply_only() && $current_user_control->is_capped() ) {
		echo sanitize_text_field( __( 'Your messaging is set to Reply Only, you can only message users who have previously sent you a message, in addition you are only able to send another ', 'bp-messaging-control' ) ) . $current_user_control->get_capped_limit() . sanitize_text_field( __( ' messages this ', 'bp-messaging-control' ) ) . $settings['quota_time_setting']. ' '  . $character_message;
	} else if ( $current_user_control->is_capped() ) {
		echo sanitize_text_field( __( 'Your messaging is capped, you can send a further ', 'bp-messaging-control' ) ) . $current_user_control->get_capped_limit() . sanitize_text_field( __( ' messages this ', 'bp-messaging-control' ) ) . $settings['quota_time_setting'] . ' '  . $character_message;
	} else {
		echo $character_message;
	}

<?php
/**
 * BuddyxPro\BuddyxPro\Comments\Component class
 *
 * @package buddyxpro
 */

namespace BuddyxPro\BuddyxPro\Comments;
 ...
 * * `buddyxpro()->the_comments( array $args = [] )`
 *
	...
	public function the_comments( array $args = [] ) {
		$args = array_merge(
			...
			[
				'style'      => 'ol',
				'short_ping' => true,
				'callback' => 'buddyx_pro_comments_callback',
			]
		);

		$amp_live_list = buddyxpro()->using_amp_live_list_comments();
		...
		if ( $amp_live_list ) {
			...
			?>
				<div update>
					<button class="button" on="tap:amp-live-comments-list-<?php the_ID(); ?>.update"><?php esc_html_e( 'New comment(s)', 'buddyxpro' ); ?></button>
				</div>
			</amp-live-list>
			<?php
		}

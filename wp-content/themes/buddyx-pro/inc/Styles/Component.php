<?php
/**
 * BuddyxPro\BuddyxPro\Styles\Component class
 *
 * @package buddyxpro
 */

namespace BuddyxPro\BuddyxPro\Styles;
...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function initialize() {
		...
		add_action( 'wp_head', [ $this, 'load_custom_styles' ] );
	}
	...
	public function action_enqueue_styles() {
		...
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'buddyxpro-fonts', $google_fonts_url, [], null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		foreach ( $css_files as $handle => $data ) {
			...
			$version = buddyxpro()->get_asset_version( $css_dir . $data['file'] );
	...
	public function filter_resource_hints( array $urls, string $relation_type ) : array {
		if ( 'preconnect' === $relation_type && wp_style_is( 'buddyxpro-fonts', 'queue' ) ) {
			$urls[] = [
		...
		$handles   = array_filter(
			...
			function( $handle ) use ( $css_files ) {
				...
				if ( ! $is_valid ) {
					...
					_doing_it_wrong( __CLASS__ . '::print_styles()', esc_html( sprintf( __( 'Invalid theme stylesheet handle: %s', 'buddyxpro' ), $handle ) ), 'BuddyxPro 2.0.0' );
				}
				return $is_valid;
			}
		);
	 ...
	 * {@see 'buddyxpro_preloading_styles_enabled'} filter can be used to tweak the return value.
	 *
	...
	protected function preloading_styles_enabled() {
		$preloading_styles_enabled = ! buddyxpro()->is_amp();
		 ...
		 */
		return apply_filters( 'buddyxpro_preloading_styles_enabled', $preloading_styles_enabled );
	}
	...
	protected function get_css_files() : array {
		...
		$css_files = [
			...
			'buddyxpro-site-loader' => [
				'file' => 'loaders.min.css',
				'global' => true,
			],
			'buddyxpro-load-fontawesome' => [
				'file' => 'fontawesome.min.css',
				'global' => true,
			],
			'buddyxpro-woocommerce' => [
		 ...
		 */
		$css_files = apply_filters( 'buddyxpro_css_files', $css_files );
	...
	protected function get_google_fonts() : array {
		...
		$google_fonts = [
			'Nunito Sans'  => [ 'ital,wght@0,200;0,300;0,400;0,600;0,700;0,800;0,900;1,undefined,300;1,400;1,600&display=swap' ],
		];
		 ...
		 */
		$this->google_fonts = (array) apply_filters( 'buddyxpro_google_fonts', $google_fonts );
	...
	public function load_custom_styles() {
		...
		$output = buddyx_get_custom_style();

<?php
/**
 * BuddyxPro Profile Completion Widget.
 */
...
class BP_BuddyxPro_Profile_Completion_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array(
			...
			'description' => __( 'Show Logged in user Profile Completion Progress.', 'buddyxpro' ),
		);

		/* Set up the widget. */
		parent::__construct(
			false,
			__( '(BuddyPress) Profile Completion', 'buddyxpro' ),
			$widget_ops
		);
	}
	...
	function widget( $args, $instance ) {
		...
		$progress_label = sprintf( esc_html__( '%s Complete', 'buddyxpro' ), $user_progress['completion_percentage'] . '%' );
		?>
				...
				<div class="wb-bp-progress-text">
					<div class="wb-bp-user-avatar-wrap">
						...
						<div class="wb-bp-progress-label">
							...
							<span><?php echo esc_html__( 'Complete', 'buddyxpro' ); ?></span>
						</div>
					</div>
				</div>
	...
	function get_user_profile_progress( $group_ids, $photo_types ) {
		...
		if ( ! $is_profile_photo_disabled && in_array( 'profile_photo', $photo_types ) ) {
			...
			$progress_details['photo_type']['profile_photo'] = array(
				...
				'name'        => __( 'Profile Photo', 'buddyxpro' ),
			);
		}
		...
		$progress_details['groups'][] = array(
			'group_name'             => __( 'Profile Fields', 'buddyxpro' ),
		...
		return apply_filters( 'BP_buddyxPro_user_progress', $progress_details );
	}
	...
	function get_user_profile_progress_formatted( $user_progress_arr ) {
		 ...
		 */
		return apply_filters( 'bp_buddyxpro_user_progress_formatted', $user_prgress_formatted );
	}
	...
	function update( $new_instance, $old_instance ) {
		return apply_filters( 'bp_buddyxpro_profile_completion_form_update', $new_instance );
	}
	...
	function form( $instance ) {
		...
		$instance = wp_parse_args(
			(array) $instance,
			array(
				'title'                  => __( 'Complete Your Profile', 'buddyxpro' ),
		...
		if ( ! $is_profile_photo_disabled ) {
			$photos_enabled_arr['profile_photo'] = __( 'Profile Photo', 'buddyxpro' );
		}
		if ( ! $is_cover_photo_disabled ) {
			$photos_enabled_arr['cover_photo'] = __( 'Cover Photo', 'buddyxpro' );
		}

		/* Widget Form HTML */
		?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title:', 'buddyxpro' ); ?></label>
			<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $instance['title'] ); ?>"/>
		</p>        <p>
			<label><?php esc_html_e( 'Profile field sets:', 'buddyxpro' ); ?></label>
	...
	<?php endif; ?>
		...
		<?php if ( ! empty( $photos_enabled_arr ) ) : ?>
			<p>
				<label><?php esc_html_e( 'Profile photos:', 'buddyxpro' ); ?></label>
		...
		<p>
			...
			<?php esc_html_e( 'Hide widget once progress hits 100%', 'buddyxpro' ); ?>
			</label>

		</p>
		<p>
			<small>
		<?php esc_html_e( 'Note: This widget is only displayed if a member is logged in.', 'buddyxpro' ); ?>
			</small>
		</p>
...
add_action(
	...
	function () {
		if ( class_exists( 'BuddyPress' ) ) {
			register_widget( 'BP_BuddyxPro_Profile_Completion_Widget' );
		}
	}
);

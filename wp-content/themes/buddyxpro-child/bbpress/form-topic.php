...
if ( !bbp_is_single_forum() ) :
	...
	<div id="buddyx-forums" class="buddyx-wrapper">
	...
	<?php if ( bbp_current_user_can_access_create_topic_form() ) : ?>
		...
		<div id="new-topic-<?php bbp_topic_id(); ?>" class="bbp-topic-form">
			...
			<form id="new-post" name="new-post" method="post" class="bx-bbp-form">
				...
				<fieldset class="bbp-form">
					<legend>
						...
						if ( bbp_is_topic_edit() ) :
							printf( esc_html__( 'Now Editing &ldquo;%s&rdquo;', 'bbpress' ), bbp_get_topic_title() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						else :
							( bbp_is_single_forum() && bbp_get_forum_title() ) ? printf( esc_html__( 'Create New Topic in &ldquo;%s&rdquo;', 'bbpress' ), bbp_get_forum_title() ) : esc_html_e( 'Create New Topic', 'bbpress' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						endif;
						?>
					...
					<div>
						...
						<p>
							<label for="bbp_topic_title"><?php printf( esc_html__( 'Topic Title (Maximum Length: %d):', 'bbpress' ), bbp_get_title_max_length() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></label>
						...
						<?php if ( bbp_allow_revisions() && bbp_is_topic_edit() ) : ?>
							...
							<fieldset class="bbp-form">
								...
								<div>
									<label for="bbp_topic_edit_reason"><?php printf( esc_html__( 'Optional reason for editing:', 'bbpress' ), bbp_get_current_user_name() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></label>
	...
	<?php elseif ( bbp_is_forum_closed() ) : ?>
		...
		<div id="forum-closed-<?php bbp_forum_id(); ?>" class="bbp-forum-closed">
			<div class="bbp-template-notice">
				<ul>
					<li><?php printf( esc_html__( 'The forum &#8216;%s&#8217; is closed to new topics and replies.', 'bbpress' ), bbp_get_forum_title() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></li>
				</ul>
			</div>
		</div>
	...
	<?php else : ?>
		...
		<div id="no-topic-<?php bbp_forum_id(); ?>" class="bbp-no-topic">
			<div class="bbp-template-notice">
				<ul>
					<li><?php
						is_user_logged_in() ? esc_html_e( 'You cannot create new topics.', 'bbpress' ) : esc_html_e( 'You must be logged in to create new topics.', 'bbpress' );
						?></li>
				</ul>
			</div>
